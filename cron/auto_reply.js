...
// Detects comments cut off mid-sentence (MiMo sometimes stops early — real
    ...
    const result = await mimoKeys.execute(async (apiKey) => {
      const res = await fetch(process.env.MIMO_API_URL || 'https://api.xiaomimimo.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          // MiMo is a reasoning model — disable thinking so it cannot burn the
          // token budget on reasoning_content and return an empty comment.
