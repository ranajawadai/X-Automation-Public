...
export async function postToQueue(text, imageUrl = null) {
  ...
  // F1c: Reject leaked meta-text captions (numbered labels, field labels, meta-descriptions)
  if (/^\d+\.\s*(emoji|content|hook|body|cta|intent|tone|style|rewrite|translate|title|char\s*(limit|count|s)?)\s*:/i.test(text) || /^(intent|content|emoji|hook|body|cta|tone|style|char\s*(limit|count|s)?)\s*:/i.test(text) || /^(the post is|this post (is|shows|contains)|this is a screenshot of|this is likely about|this image shows|the image shows)\b/i.test(text)) {
